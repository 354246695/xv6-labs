...
uint64 
acquire_nproc(){
  struct proc *p; // 定义一个指向进程结构的指针p
  uint64 cnt = 0; // 初始化计数器cnt为0，用于计数非空闲进程
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    acquire(&p->lock); // 获取进程p的锁，确保线程安全
    if (p->state != UNUSED)  // 判断该进程的状态是否不是UNUSED（未使用）
    {
      cnt++; // 如果进程正在被使用，计数器cnt加1
    }

    release(&p->lock); // 释放进程p的锁
  }

  return cnt; // 返回非空闲进程的总数
}
